...
from dinov2.data import collate_data_and_cast, DataAugmentationDINO, MaskingGenerator, collate_tile_embs_and_cast, EmbMaskingGenerator
...
def do_train(cfg, model, resume=False):
    ...
    for data in metric_logger.log_every(
        ...
        if math.isnan(sum(loss_dict_reduced.values())):
            ...
            #     logger.info(f"{k}: {v}")
            # #show the idx and data
            # logger.info(f"idx: {idx}")
            # logger.info(f"data: {data['collated_global_crops'].shape}")
            # logger.info(f"local crops: {data['collated_local_crops'].shape}")
            # logger.info(f"data: {data} ")
            #change the nan loss to 0

            # #store the data into a file
            # import pickle
            # with open('error_data'+str(idx)+'.pkl', 'wb') as f:
            #     pickle.dump(data, f)
            logger.info("NaN detected")
            raise AssertionError

        losses_reduced = sum(loss for loss in loss_dict_reduced.values())
        # #print data types of the loss_dict_reduced
        # for k, v in loss_dict_reduced.items():
        #     #check float32 or float16
        #     logger.info(f"{k}: {v} type: {v.dtype}")
