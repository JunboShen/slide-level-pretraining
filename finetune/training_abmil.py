...
from abmil.model import Attention
...
def train(dataloader, fold, args):
    ...
    # set up the model
    model = Attention(input_dim=1536, n_classes= args.n_classes)
    model = model.to(args.device)
    # set up the optimizer
    optim_func = torch.optim.AdamW if args.optim == 'adamw' else torch.optim.Adam
    optimizer = optim_func(model.parameters(), lr=0.0005, weight_decay=1e-5)
