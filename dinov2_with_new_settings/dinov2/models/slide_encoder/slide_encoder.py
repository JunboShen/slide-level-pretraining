...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        # self.proj1 = nn.Linear(in_chans, embed_dim // 2, bias=bias)
        # self.act = nn.GELU()  # Non-linear activation
        # self.proj2 = nn.Linear(embed_dim // 2, embed_dim, bias=bias)
        #self.norm_before = nn.LayerNorm(in_chans, eps=1e-6)
...
class LongNetViT(nn.Module):
    ...
    def forward_features(self, x, coords, masks=None, all_layer_embed=False):
        ...
        for x in x_list:
            x_norm = self.norm(x)
            #x_norm_patchtokens_avg = x_norm[:, self.num_register_tokens + 1 :].mean(dim=1)
            outcomes.append(
                {
                    "x_norm_clstoken": x_norm[:, 0],
                    "x_norm_regtokens": x_norm[:, 1 : self.num_register_tokens + 1],
