...
class LongNetViT(nn.Module):
    ...
    def forward(self, x, coords, all_layer_embed=False):
        ...
        #9.7
        # for x in x_list:
        #     # global_pool = x[:, 1:, :].mean(dim=1)
        #     # global_pool = self.norm(global_pool)
        #     x = self.norm(x)
        #     cls_token = x[:, 0]
        #     global_pool = x[:, 1:].mean(dim=1)  # global average pooling
        #     #concatenate the cls token and the global pool
        #     outcome = torch.cat((cls_token, global_pool), dim=1)
